...
class FriendshipResourceIT {
    ...
    void getAllFriendshipsWithEagerRelationshipsIsEnabled() throws Exception {
        when(friendshipRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restFriendshipMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(friendshipRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllFriendshipsWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(friendshipRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restFriendshipMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(friendshipRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
